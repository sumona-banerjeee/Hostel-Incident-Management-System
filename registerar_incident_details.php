...
<body>
    <div class="container">
        ...
        <table class="highlight">
            ...
            <tr><th>Number of Days</th><td><?php echo $incident['Fine']; ?></td></tr>
        ...
        <div class="button-row">
            <a href="registerardashboard.php" class="btn left-btn">Back to Dashboard</a>

            <?php if ($incident['Status'] === 'Pending Registrar Approval'): ?>
                <button class="btn right-btn" id="approveRegistrarBtn">Approve</button>
            <?php endif; ?>
        </div>


        <?php if ($incident['Status'] === 'Pending Registrar Approval'): ?>
            <p id="approvalMsg" style="color: green; display: none; margin-top: 10px;">Rustication approved and parent notified.</p>
        <?php endif; ?>
    </div>

    <script>
        document.getElementById("approveRegistrarBtn")?.addEventListener("click", function () {
            var incidentId = <?php echo $incident['IncidentID']; ?>;

            fetch("registrar_approve.php?id=" + incidentId, {
                method: "GET"
            })
            ...
            .then(data => {
                if (data.trim() === "success") {
                    ...
                    document.getElementById("approveRegistrarBtn").style.display = "none";
                } else {
                    alert("Error: " + data);
                }
            })
